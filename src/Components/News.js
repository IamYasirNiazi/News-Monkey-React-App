...
const News = (props)=>{
  ...
  // articles= [
    ...
    useEffect(() => {
      ...
      document.title = `${CapCategoryTitle(props.category)} News | NewsMonkey`;
    ...
    const fetchMoreData = async ()=>{
      ...
      // ${page + 1} Alone This not worked because it gives repeated news on page 1 & 2. this happens because set is an async function and therefore it takes time to manipulate.

      // Two ways to overcome this Error
      // 1. ${++page} and at the end setPage(page) Fixed the code using this.
      
      // 2. Manually set the page like ${page + 1} because url dont have to wait for the value bacause we set the value manually and then at the we set the value by setPage(page + 1)
    ...
    return (
      <>
        ...
        {/* <div className="container py-2 mx-auto d-flex flex-column justify-content-center"> */}
            ...
            <h1 className='text-center fw-bold my-4 pb-1' style={{paddingTop: '3.5rem'}}>NewsMonkey | Top {CapCategoryTitle(props.category)} Headlines</h1>
            ...
            <button type="button" className="btn btn-sm btn-primary me-3" disabled={page<=1} onClick={prevBtnFunc} >&larr; Prev</button>
            <button type="button" className="btn btn-sm btn-primary" id="next-button" disabled={page + 1>Math.ceil(totalResults/props.pageSize)} onClick={nextBtnFunc} >Next &rarr;</button>
